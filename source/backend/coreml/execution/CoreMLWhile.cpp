    ...
    ErrorCode CoreMLWhile::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
        ...
        if(whileLoopParam->tensorNumber() > 0) {
            if(inputs[whileLoopParam->tensorNumber()] != nullptr) {
                const char* conditionVarName = mCoreMLBackend->getTensorName(inputs[whileLoopParam->tensorNumber()]).c_str();
                mLayer_->loop->conditionvar = mCoreMLBackend->create<char>(strlen(conditionVarName) + 1);
